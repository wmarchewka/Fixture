...
def CycloneProgram(self, port, image):
        ...
        try:
            logger.debug('Looking for Cyclone programmer on port ' + str(port))
            self.lblStatus.setText('Looking for Cyclone programmer on port '+ str(port))
            time.sleep(1)
        ...
        if line == b'':
            self.lblStatus.setText('Cyclone programmer not responding')
            return False, 'Cyclone programmer not responding'
